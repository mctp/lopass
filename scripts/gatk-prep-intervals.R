...
tmp <- fread("data/external/wgs_calling_regions.hg38.interval_list", skip = 3368)
...
firstX <- min(which(seqnames(rng)=="chrX"))
rng[firstX:(par1.split-1)]$xy.ploidy <- 1
...
fwrite(rng.tbl, "./hg38/intervals/wgs-npartition-ploidy.tsv", sep="\t", col.names=FALSE)
